...
const usersReducer = (state = initialState, action) => {
    ...
    switch(action.type){
            ...
            return{
                ...
                users: state.users.map((u) => {
                    if (u.id === action.userID) {
                        return { ...u, statusFriend: true }; //заменяем в statusFriend на true
                    }
                    return u;
                }
                )
        ...
        case SET_USERS: 
            return {...state, users: [...state.users, ...action.users]} //склеиваем два массива, тот который был и тот который прищёл
        default:
            return state;
    }   
};
